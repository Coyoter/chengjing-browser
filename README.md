...
目前原始碼版本為 **1.4.0（versionCode 21）**。新增下載管理、可搜尋的歷史記錄、一般／無痕雙分頁集，以及簡潔分頁卡片。**第一層天眼工具列與網址列外觀維持 1.3.1，分享仍只在三點選單內。** 安裝檔以 GitHub Releases 與對應工作流程的成功結果為準。[1.4.0 更新說明](docs/releases/1.4.0.md) · [簽章與發布流程](docs/RELEASING.md)

無痕使用獨立的 WebView 設定檔，不保存歷史、搜尋詞或可恢復分頁，並在工作階段結束時清理網站資料。不支援隔離／清理能力的 WebView 會要求更新，沒有共用 Cookie 的降級模式。隔離設定檔可能暫時寫入裝置儲存，並非匿名或純記憶體保證；完整說明見 App 內隱私政策。
