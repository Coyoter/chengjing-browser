...
> 此修正分支尚未發布新版 APK/AAB：包含 OpenRouter 設定即時更新、收藏與已儲存閱讀進度同步，以及參考澄境筆記的原生啟動畫面。既有 1.2.0 安裝檔不會因原始碼更新而自動改變。以下 1.2.0 的既有驗證紀錄，不代表新修正已完成實機或 Google 真實帳戶驗收。
...
- Google 同步僅要求 `drive.appdata`。本修正分支加入收藏名稱、頁面標題、網址、星號保存的閱讀位置與百分比，以及刪除狀態；兩端都須使用包含此修正的版本。收藏以最後儲存的狀態合併，不取最大的閱讀百分比，也不在同步時跳動目前正在閱讀的頁面。天眼網站設定同步仍預設關閉，開啟後同步元件規則、自訂 CSS／JS／HTML 與網站行為設定。瀏覽紀錄、Cookie、API Key、本機模型與憑證例外不加入同步。
...
GitHub Actions 使用 `-PqaInstall=true` 建置獨立的 `tw.techtarian.browser.qa` 測試套件，不使用正式簽章或 Google 帳戶。流程執行既有與新增 JVM 測試，再執行 OpenRouter 切換、兩份隔離收藏儲存區合併，以及深淺色啟動畫面渲染測試。測試結果以對應 commit 的 Actions 實際紀錄為準；工作流程存在不等於測試已通過。收藏的 Google 真實帳戶往返、兩支實體手機與正式簽章更新仍需另外驗收。
