...
1. 安裝 `release/ChengJing-Browser-0.1.14-Android.apk`，Android 9 或以上。
...
設定 → 瀏覽中附有本機練習場，可以測試移除、同網域第二頁與例外切換。天眼的規則、AI、自訂程式碼入口在選單的「天眼設定」。
...
- 快速收藏星號依目前網址是否已保存，顯示空心或實心；重開同一網址、上一頁／下一頁，以及網站不重新載入就切換章節時，也會更新狀態。從收藏讀到新章節時先顯示空心，按星號更新進度後變實心。
- 主選單移除頂端拖曳橫槓，標題留在系統狀態列／前置鏡頭區域下方。
- Android 自適應圖示的圖案由原始大小的 78% 再縮至 60%（比 0.1.7 再小約 23%），首頁與相容圖示也調整留白；開啟 App 時明確指定最近工作項目的目前圖示。


## 0.1.9：選單與設定重新設計

- 主選單改成四個等寬快捷入口，以及「目前頁面、天眼與網站、偏好與資料」分組。操作列統一 64 dp，長網頁／收藏名稱以單行省略顯示。
- 設定分成固定的「外觀、瀏覽、AI」分類；外觀以等寬主題選項、網址列上下位置示意圖呈現，AI 分成金鑰連線與模型。瀏覽器識別與練習場收在「設定 → 瀏覽」。
- 主選單與設定使用一致的固定面板，分類切換時外框不跳動，只捲動內容；所有輔助面板沿用相同寬度、邊距、返回與關閉。淺色與深色共用同一套間距與排版。
- 面板避開系統頂端區域，最高採可用高度的 92%；沒有拖曳橫槓，可按關閉、返回鍵或面板外離開。

完整設計決策見 [選單設計](docs/MENU-DESIGN-019.md)。

設定頁底部新增「瀏覽核心」，顯示目前 Chromium、對應元件的 Google 官方最新正式版、比較結果與查詢時間；可手動重新檢查或開啟核心更新頁。自動查詢結果保留 6 小時，失敗時顯示狀態並保留上次結果。版本來源為官方 VersionHistory API，與自訂 User-Agent 分開。

選不到的特殊元素，請見 [處理方式與限制](docs/SPECIAL-ELEMENTS.md)。


## 0.1.10：全螢幕選單

選單、設定與輔助頁面改為鋪滿螢幕的完整頁面，移除 92% 高度限制、左右及底部外圍空隙，以及浮動視窗圓角。分組、等寬快捷入口與統一列高保留；內容避開狀態列、前置鏡頭及手勢列，並支援鍵盤。以返回或右上角關閉離開。

核心仍使用手機的系統 WebView，不內嵌或假冒 154。系統與官方商店若未向該裝置提供新版，更新澄境 APK 不會替換手機核心。


## 0.1.11：網址列新增分頁按鈕

網址列與分頁數字之間新增「＋」，點一下直接開啟空白新分頁並切換過去；原分頁保留。上下兩種網址列配置共用同一個按鈕，觸控範圍 48 dp、圖示 24 dp，與分頁框及網址列垂直置中。


## 0.1.12：網址列整排內縮

網址列、＋與分頁按鈕整排的左右邊距，由各 8 dp 加大為各 24 dp，為手機下方圓角留出更多空間。上下兩種網址列配置皆套用。


## 0.1.13：編輯書籤網址

書籤編輯視窗包含「名稱、網址、資料夾」，開啟時顯示原本網址，可直接修改並儲存。支援完整 HTTP／HTTPS 網址，格式不正確時顯示提示；取消不變更原書籤。網址修改沿用既有同步格式，保留舊項目的刪除記錄，避免舊快照重新帶回原網址。


## 0.1.14：移除首頁天眼教學卡

首頁移除「先試一次天眼」整張卡片與卡片下方留白，快速前往及繼續閱讀直接往上銜接。練習場仍可從「設定 → 瀏覽 → 天眼練習場」開啟。
