...
1. 安裝 `release/ChengJing-Browser-0.1.7-Android.apk`，Android 9 或以上。
2. 開啟網站，按工具列「天眼」，直接進入選取模式。
3. 點中元件。若只選到文字或小叉叉，按「選取外面一層」。
4. 按「預覽移除」，確認文章仍正常後按頂部「儲存」。按 × 會取消未儲存的預覽。
5. 選單「暫時顯示原始網站」切換例外；選「恢復套用天眼規則」恢復。天眼右側星號可快速收藏目前頁面、再次點擊更新閱讀進度。
...
- 書籤頁右上角「匯入 Chrome 書籤」可選取 HTML；保留資料夾、去重，完成後直接顯示書籤。支援匯出、搜尋與編輯；Google Drive 書籤同步已接入並完成真實雲端讀回。
...
- 天眼右側改為快速收藏星號；點一下保存目前頁面與位置，已有連結收藏則更新同一筆進度。
- 「暫時顯示原始網站／恢復套用天眼規則」移到主選單，天眼設定內仍可操作。
- Chrome HTML 匯入只保留在書籤頁右上角。
- 例外切換直接用新設定重載該網域；避免在即將離開的舊頁面反覆套用規則，也不重新套用其他網域目前正在顯示的頁面。未進入天眼時，普通導覽不再多做一次結束選取與重套規則。
