...
class BrowserController(val context: Context, val store: BrowserStore) {
    ...
    val incognito:Boolean get()=active?.incognito==true
    var tabGroupPrivate by mutableStateOf(false)
    ...
    fun captureActivePreview() {
        ...
        if(tab.incognito||tab.url.isBlank()||!tab.web.isAttachedToWindow||tab.web.width<=0||tab.web.height<=0)return
        runCatching {
            val width=minOf(320,tab.web.width)
            val factor=width.toFloat()/tab.web.width
            val image=Bitmap.createBitmap(width,minOf(220,(tab.web.height*factor).toInt()).coerceAtLeast(1),Bitmap.Config.RGB_565)
            val canvas=android.graphics.Canvas(image)
            canvas.scale(factor,factor)
            tab.web.draw(canvas)
            tab.preview=image // Memory only. Dropped on navigation/close; never serialized.
        }
    }
    fun showTabs() {captureActivePreview();tabGroupPrivate=incognito;sheet="tabs"}
    fun closeGroup(privateMode:Boolean) {tabs.filter{it.incognito==privateMode}.map{it.id}.forEach{closeTab(it)}}
    ...
    fun newTab(url: String = "", incognito:Boolean=this.incognito): BrowserTab? {
        if(tabs.size>=20){notice="目前最多可開啟 20 個分頁，請先關閉不用的分頁";return null}
        if(incognito&&!privateSession.available){notice="請先更新 Android System WebView／Chrome，才能使用安全隔離的無痕分頁";return null}
        runCatching{PrivateSession.sweepAbandoned()}
        captureActivePreview()
        stopEye()
        val web=SelectionWebView(context)
        if(incognito)try{privateSession.attach(web)}catch(_:Exception){web.destroy();notice="無法建立隔離工作階段，未開啟無痕分頁";return null}
        // Discard WebView trust decisions from older versions; only saved site choices apply.
        web.clearSslPreferences()
        val tab=BrowserTab(nextId++,web,incognito)
        ...
        PrivateSession.cookies(web).setAcceptThirdPartyCookies(web,false)
        if(incognito){web.settings.cacheMode=WebSettings.LOAD_NO_CACHE;web.importantForAutofill=android.view.View.IMPORTANT_FOR_AUTOFILL_NO_EXCLUDE_DESCENDANTS}
        ...
        web.webViewClient=object:WebViewClient(){
            ...
            override fun onPageFinished(view:WebView,url:String) {
                ...
                if(tab in tabs&&tab.error.isEmpty()&&!tab.incognito) store.visit(url,tab.title)
        ...
        web.setDownloadListener { url, userAgent, disposition, mime, _ ->
            ...
            android.app.AlertDialog.Builder(context).setTitle("下載檔案？").setMessage(name).setNegativeButton("取消",null).setPositiveButton("下載") { _,_->
                (context as? MainActivity)?.imageDownloads?.download(url,tab.url,userAgent,tab,disposition,mime)
            }.show()
        }
    ...
    fun navigate(input:String,fromFavorite:Favorite?=null) {
        ...
        if(!incognito)store.recordSearch(input,url);revision++
    ...
    suspend fun saveFavorite(forceNew:Boolean=false):Favorite?{
        if(incognito){notice="無痕瀏覽不保存收藏進度，請在一般分頁中收藏";return null}
    ...
    fun setCertificateException(origin:String,enabled:Boolean){
        if(incognito){notice="無痕分頁不保存新的憑證例外，請改用一般分頁設定";return}
    ...
    fun switchTab(id:Int){if(tabs.none{it.id==id})return;captureActivePreview();stopEye();active?.web?.onPause();activeId=id;active?.web?.onResume();sheet=""}
    fun closeTab(id:Int){
        if(id==activeId)stopEye()
        val tab=tabs.find{it.id==id}?:return
        tab.preview=null
        tab.web.stopLoading();tab.documentScript?.remove()
        ...
        if(tab.incognito&&tabs.none{it.incognito})privateSession.finish()
    ...
    fun destroy(){exitFullscreen();gemma.close();icons.close();fileCallback?.onReceiveValue(null);tabs.forEach{
        it.preview=null;it.web.stopLoading();it.documentScript?.remove()
        (it.refreshContainer.parent as? ViewGroup)?.removeView(it.refreshContainer);it.refreshContainer.removeAllViews();it.web.destroy()
    };tabs.clear();privateSession.finish()}
}
