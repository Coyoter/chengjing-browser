...
class BookmarkSync(private val activity:MainActivity,private val store:BookmarkStore){
        ...
        tasks.launch{
            mutex.withLock{
                try{
                    ...
                    val favoriteFiles=withContext(Dispatchers.IO){drive.files(FavoriteSyncFormat.TAG)}
                    val favoriteRemote=withContext(Dispatchers.IO){
                        FavoriteSyncFormat.merge(*favoriteFiles.map{FavoriteSyncFormat.read(drive.readRaw(it.getString("id")))}.toTypedArray())
                    }
                    check(version==epoch){"連線已取消"}
                    if(favorites.mergeRemote(favoriteRemote))activity.controller.revision++
                    val favoriteUpload=FavoriteSyncFormat.write(favorites.syncRecords())
                    val favoriteOwn=favoriteFiles.find{it.optJSONObject("appProperties")?.optString("device")==store.deviceId}?.getString("id")
                    val favoriteFileId=withContext(Dispatchers.IO){drive.writeRaw(store.deviceId,favoriteOwn,favoriteUpload,FavoriteSyncFormat.TAG)}
                    check(version==epoch){"連線已取消"}
                    val favoriteVerified=withContext(Dispatchers.IO){FavoriteSyncFormat.write(FavoriteSyncFormat.read(drive.readRaw(favoriteFileId)))}
                    check(version==epoch){"連線已取消"}
                    ...
                    details=""
                    // Changes made after any upload snapshot must get another round, not be dropped while busy.
                    if(sitesChangedWhileUploading||FavoriteSyncFormat.write(favorites.syncRecords())!=favoriteUpload||BookmarkFormat.snapshot(store.all())!=BookmarkFormat.snapshot(upload))changed()
    ...
    fun destroy(){epoch++;currentDrive?.cancel();tasks.cancel();store.onChange=null;activity.controller.favorites.onChange=null;activity.controller.store.onSiteChange=null}
}
