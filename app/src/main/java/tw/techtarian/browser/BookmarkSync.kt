    ...
    private fun request(url:String,method:String="GET",body:RequestBody?=null):String{
        client.newCall(Request.Builder().url(url).header("Authorization","Bearer $token").method(method,body).build()).execute().use{r->
            check(r.isSuccessful){when(r.code){
                ...
                else->throw SyncHttpException(r.code,"資料同步未完成（HTTP ${r.code}），本機資料已保留")
            }}
...
class BookmarkSync(private val activity:MainActivity,private val store:BookmarkStore){
    ...
    init{
        ...
        activity.controller.favorites.onChange={changed()}
    }
    ...
    fun consent(data:android.content.Intent?){
        ...
        runCatching{Identity.getAuthorizationClient(activity).getAuthorizationResultFromIntent(data)}.onSuccess{accept(it,version)}.onFailure{
            if(it is ApiException){
                ...
                fail("已取消連結；本機書籤與收藏保持不變",detail=it.localizedMessage)
            }
        }
    }
        ...
        tasks.launch{
            mutex.withLock{
                try{
                    ...
                    check(store.accountId.isEmpty()||store.accountId==id){"此手機的書籤與收藏已綁定另一個 Google 帳戶；請使用原本帳戶，避免混入其他人的資料"}
                    ...
                    status="正在同步收藏與閱讀進度…"
                    val favorites=activity.controller.favorites
                    val favoriteFiles=withContext(Dispatchers.IO){drive.files(FavoriteFormat.TAG)}
                    val favoriteRemote=withContext(Dispatchers.IO){
                        FavoriteFormat.merge(*favoriteFiles.map{FavoriteFormat.read(drive.readRaw(it.getString("id")))}.toTypedArray())
                    }
                    check(version==epoch){"連線已取消"}
                    if(favorites.mergeRemote(favoriteRemote)){
                        // Do not move a page the user is reading. Only detach deleted identities.
                        activity.controller.tabs.forEach{tab->
                            if(tab.favoriteId!=null && favorites.get(tab.favoriteId!!)==null){
                                tab.favoriteId=null;tab.favoriteRestore=null
                            }
                        }
                        activity.controller.persistTabs()
                        activity.controller.revision++
                    }
                    val favoriteUpload=FavoriteFormat.write(favorites.records())
                    val favoriteOwn=favoriteFiles.find{it.optJSONObject("appProperties")?.optString("device")==store.deviceId}?.getString("id")
                    val favoriteFileId=withContext(Dispatchers.IO){drive.writeRaw(store.deviceId,favoriteOwn,favoriteUpload,FavoriteFormat.TAG)}
                    check(version==epoch){"連線已取消"}
                    val favoriteVerified=withContext(Dispatchers.IO){FavoriteFormat.write(FavoriteFormat.read(drive.readRaw(favoriteFileId)))}
                    check(version==epoch){"連線已取消"}
                    check(favoriteVerified==favoriteUpload){"收藏與進度寫入回讀不一致，請重試"}
                    ...
                    details=""
                    // Include changes made while *any* later collection was being uploaded.
                    if(sitesChangedWhileUploading || BookmarkFormat.snapshot(store.all())!=BookmarkFormat.snapshot(upload)
                        || FavoriteFormat.write(favorites.records())!=favoriteUpload)changed()
    ...
    fun disconnect(){epoch++;consentEpoch=null;currentDrive?.cancel();currentDrive=null;pending?.cancel();store.connected=false;connected=false;busy=false;authorizing=false;status="已停止同步，本機與雲端資料均保留"}
    fun destroy(){epoch++;currentDrive?.cancel();tasks.cancel();store.onChange=null;activity.controller.store.onSiteChange=null;activity.controller.favorites.onChange=null}
}
