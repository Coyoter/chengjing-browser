...
visual-language: 澄境暖白／墨綠表面、玉綠狀態色、雙欄精簡選項
mode: extension
visual-variance: 2
motion-intensity: 1
...
延續既有色票與系統字體：16sp 選項名稱、11–12sp 網域與輔助文字。8dp 網格間距、16dp 選項圓角、18dp 群組圓角、1dp 細框線及至少 76dp 操作高度。上方精簡摘要回答「現在用哪一個」，六個內建服務排成雙欄。選中項目以玉綠底色、框線和單選指示共同標示。使用名稱與網域兩層文字，不另載外部商標；不增加陰影或動畫。

自訂模式使用可換行的網址欄位，避免長範本只顯示尾端。`{query}` 是唯一需要理解的規則，輔助文字、即時錯誤、編碼後預覽與「儲存並使用」形成同一流程。錯誤內容不會改變目前使用的搜尋引擎。
自訂編輯器獨立收納，只有儲存成功才更新單選狀態；展開或輸入時仍清楚標示原本正在使用的引擎。
