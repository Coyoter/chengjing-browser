...
- 網站全部權限、Chrome 全功能相容、DRM、惡意網站刻意對抗、所有網站元件系統均沒有完整涵蓋。
...
## 1.0.0 候選版：永久網站憑證例外與天眼程式碼入口

- 預設取消可恢復的 SSL 憑證錯誤；明確手動同意後，永久保存該 HTTPS 主機名稱的例外，重開及憑證更換後仍有效，直到使用者關閉。啟用時持續顯示警示，不加入書籤同步。
- 實測 WebView 的 TLS 例外快取依主機名稱共用，無法只靠回呼承諾隔離同主機的不同連接埠。因此實作與介面明確採「同 HTTPS 主機、跨連接埠」範圍，不包含其他主機或子網域；沒有宣稱測試失敗的連接埠隔離已修復。
- 修正首次 SSL 失敗未收到 onPageStarted 時丟失網址、連線圖示與重試入口的問題。新建 WebView 清除舊版隱含的 SSL 記憶，僅依明確保存的網站設定放行。
- 40 個 JVM 測試通過。API 36 release 設定的 QA 套件通過 8 個操作測試，包括主頁預設阻擋、介面啟用、Activity 重建、關閉後新分頁再次阻擋、有效主頁的異常資源獨立阻擋、同主機不同連接埠行為，以及 CSS／JS 與天眼既有回歸。
- 另以兩次獨立 Android instrumentation 程序執行保存及讀回，中間 am force-stop 隔離 QA 套件；重啟後以實際自簽 TLS 文件驗證載入成功、永久開關與警示仍在。第二階段完成後撤除測試例外。
- 新增隱私與資料說明，以及每次 AI 分析前的資料傳送同意。應用、練習頁與目前專案文件的指定用途文字掃描通過；未宣稱真實付費 AI 呼叫已測試。
- 已查看連線設定與隱私面板模擬器截圖。最初截圖落在對話框淡入動畫中，等待動畫完成後重新擷取，確認背景不透明、版面完整。
- 1.0.0 APK 以既有正式簽章建置，套件 tw.techtarian.browser、versionCode 16，並已在模擬器覆蓋安裝成功。未清除原正式套件的資料；沒有實體手機驗收證據。
